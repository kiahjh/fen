...
mod tests {
    ...
    fn ints() {
        ...
        assert_response(output, r#"{"type":"success","data":42}"#);
    }
    ...
    fn strings() {
        ...
        assert_response(output, r#"{"type":"success","data":"George Washington"}"#);
    }
    ...
    fn bools() {
        ...
        assert_response(output, r#"{"type":"success","data":true}"#);
    }
    ...
    fn floats() {
        ...
        assert_response(output, r#"{"type":"success","data":42.3}"#);
    }
    ...
    fn uuids() {
        ...
        assert_response(
            output,
            r#"{"type":"success","data":"777c02a1-6555-4d08-94b5-827819b3f72c"}"#,
        );
    }
    ...
    fn dates() {
        ...
        assert_response(
            output,
            r#"{"type":"success","data":"2014-07-08T09:10:11Z"}"#,
        );
    }
    ...
    fn arrays() {
        ...
        assert_response(
            output,
            r#"{"type":"success","data":["65f4dfe8-ac5f-40bc-a64e-495727602302","7364dd48-f837-4504-8706-98e42d2c0887","f1b3b3b4-3b3b-4b3b-b3b3-b3b3b3b3b3b3"]}"#,
        );
    }
    ...
    fn optionals() {
        ...
        assert_response(output, r#"{"type":"success","data":null}"#);
    }
    ...
    fn basic_structs() {
        ...
        assert_response(
            output,
            r#"{"type":"success","data":{"name":"George Washington","age":42,"birthday":"1732-02-22T00:00:00Z","hasBeard":true}}"#,
        );
    }
    ...
    fn structs_with_compound_types() {
        ...
        assert_response(
            output,
            r#"{"type":"success","data":{"bar":[42,null,1337]}}"#,
        );

        let output: structs_with_compound_types_test::Output =
            structs_with_compound_types_test::Output { bar: vec![] };
        assert_response(output, r#"{"type":"success","data":{"bar":[]}}"#);
    }
    ...
    fn nested_structs() {
        ...
        assert_response(
            output,
            r#"{"type":"success","data":{"name":"George Washington","birthday":"1732-02-22T00:00:00Z","car":{"color":"red","year":1776}}}"#,
        );
    }
    ...
    fn basic_enums() {
        ...
        assert_response(output, r#"{"type":"success","data":{"type":"foo"}}"#);

        let output: basic_enums_test::Output = basic_enums_test::Output::Bar;
        assert_response(output, r#"{"type":"success","data":{"type":"bar"}}"#);

        let output: basic_enums_test::Output = basic_enums_test::Output::Baz;
        assert_response(output, r#"{"type":"success","data":{"type":"baz"}}"#);
    }
    ...
    fn enums_with_associated_values() {
        ...
        assert_response(
            output,
            r#"{"type":"success","data":{"type":"firstOption","data":12}}"#,
        );
        ...
        assert_response(
            output,
            r#"{"type":"success","data":{"type":"secondOption","data":["foo","bar"]}}"#,
        );
    }
    ...
    fn composing_structs_and_enums() {
        ...
        assert_response(
            output,
            r#"{"type":"success","data":{"name":"George Washington","birthday":"1732-02-22T00:00:00Z","id":"6bc9b6b8-5075-4296-af84-534e6fb5916d","car":{"color":"red","gear":{"type":"drive","data":{"type":"fifth"}}}}}"#,
        );
    }
}
